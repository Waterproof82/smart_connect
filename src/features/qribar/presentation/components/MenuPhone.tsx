  ...
  return (
    <div
      className={`flex justify-center transition-all duration-1000 ease-out ${
        isVisible
          ? "opacity-100 translate-x-0 blur-0 rotate-0"
          : "opacity-0 -translate-x-20 blur-md -rotate-6"
      }`}
    >
      <div className="relative w-full max-w-[360px] aspect-[9/19] bg-[var(--color-bg)] rounded-[3rem] border-8 border-[var(--color-surface)] shadow-lg overflow-hidden group">
        ...
        <div className="absolute top-0 w-full h-40 overflow-hidden">
          {restaurant.hasImage() && (
            <>
              ...
              />
              <div
                className="absolute inset-0 bg-gradient-to-t from-[var(--color-bg)] via-transparent"
                aria-hidden="true"
              ></div>
            </>
          )}
        ...
        <div className="mt-44 px-6 space-y-4">
          {menuItems.map((item, i) => (
            <div
              key={item.id}
              className="flex justify-between items-center py-3 border-b border-[var(--color-border)] transition-all duration-700"
              style={{
            ...
            >
              ...
              <span className="text-[var(--color-icon-amber)] font-bold text-sm">
                {item.formattedPrice}
              </span>
            </div>
          ))}
        ...
        {/* Cart button */}
        <div
          className={`absolute bottom-8 left-6 right-6 transition-all duration-700 delay-1000 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"
          }`}
        >
          <button
            type="button"
            className="w-full bg-[var(--color-text)] text-[var(--color-bg)] py-4 rounded-xl font-bold flex items-center justify-center gap-2 text-sm hover:opacity-90 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--focus-ring)]"
          >
