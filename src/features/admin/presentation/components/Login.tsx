...
const getErrorMessage = (error: string): string => {
  if (error === "Demasiados intentos. Espera unos minutos.") {
        ...
        >
          <div className="rounded-md shadow-sm space-y-4">
            <div>
              <label
                htmlFor="email"
                className="block text-xs font-semibold text-muted uppercase mb-1 ml-1"
              >
                Email{" "}
                <span
              ...
              {formErrors.email && (
                <p
                  ...
                  role="alert"
                  className="text-xs text-[var(--color-error-text)] mt-1 ml-1"
                >
          ...
          {error && (
            <div
              role="alert"
              className="rounded-lg bg-[var(--color-error-bg)] border border-[var(--color-error-border)] p-3 animate-in fade-in zoom-in duration-300"
            >
          ...
          >
            {isSubmitting ? (
              <span className="flex items-center">
                <svg
                  className="animate-spin -ml-1 mr-3 h-5 w-5 text-[var(--color-on-accent)]"
                  fill="none"
                  viewBox="0 0 24 24"
                >
                  <circle
