...
const StatCard: React.FC<StatProps> = ({ icon, label, quote, author, color, isInView, delay, prominent }) => {
  ...
  return (
    ...
    >
      <div className={`absolute -right-8 -top-8 w-32 h-32 ${color} opacity-[0.03] group-hover:opacity-[0.08] blur-2xl transition-all duration-700`}></div>
      ...
      <div className="flex flex-col items-center text-center relative z-10">
        ...
        <div className={`font-extrabold mb-3 tracking-tighter ${prominent ? 'text-4xl lg:text-5xl' : 'text-3xl'}`} aria-label={`${label}: ${count}%`}>
          <span className="text-[var(--color-primary)]">{count}%</span>
        </div>
        
        <p className="text-muted text-xs font-bold uppercase tracking-[0.2em] text-wrap mb-4">{label}</p>

        {quote && (
          <div className="relative max-w-xs">
            <Quote className="absolute -top-2 -left-1 w-4 h-4 text-[var(--color-accent)] opacity-50" />
            <p className="text-sm text-default italic pl-4 leading-relaxed">{quote}</p>
