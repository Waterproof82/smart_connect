...
describe("DeleteDocumentUseCase", () => {
  ...
  it("should delete document if user has permission and document exists", async () => {
    ...
    await expect(
      useCase.execute("doc-1", superAdminUser),
  ...
  it("should throw error if user does not have delete permission", async () => {
    mockRepository.getById.mockResolvedValue(testDocument);
    await expect(useCase.execute("doc-1", adminUser)).rejects.toThrow(
  ...
  it("should throw error if documentId is invalid", async () => {
    await expect(useCase.execute("", superAdminUser)).rejects.toThrow(
    ...
    await expect(
      useCase.execute(undefined as unknown as string, superAdminUser),
  ...
  it("should throw error if document does not exist", async () => {
    ...
    await expect(useCase.execute("doc-404", superAdminUser)).rejects.toThrow(
      "Document not found",
    );
    expect(mockRepository.delete).not.toHaveBeenCalled();
  });
