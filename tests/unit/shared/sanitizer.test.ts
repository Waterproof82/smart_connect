...
describe('sanitizeInput', () => {
  ...
  it('should throw on input exceeding maxLength', () => {
    ...
    expect(() => sanitizeInput(longInput, 'test', 4000)).toThrow('exceeds maximum length');
  });

  it('should respect custom maxLength', () => {
    expect(() => sanitizeInput('abc', 'test', 2)).toThrow('exceeds maximum length');
  });
  ...
  it('should detect XSS patterns like javascript: in HTML context', () => {
    const result = sanitizeInput('<a href="javascript:alert(1)">click</a>', 'test');
