...
const COMPONENT_PATH = path.join(
  SRC,
  "shared/components/tpv/ComprasSialtiSection.tsx",
);

describe("ComprasSialtiSection (design.md D4 bespoke module template)", () => {
  it("exists", () => {
  ...
  it("renders its own <section id> + aria-labelledby anchor matching the registry id", () => {
    ...
    expect(source).toMatch(/id="compras-sialti"/);
    expect(source).toMatch(/aria-labelledby="compras-sialti-title"/);
  });

  it("h2 carries the section id-title anchor and uses the registry titleKey", () => {
    const source = readSource();
    expect(source).toMatch(/id="compras-sialti-title"/);
    expect(source).toMatch(/t\.comprasSialtiTitle/);
  });

  it("renders a benefit lead paragraph from the registry descKey", () => {
    const source = readSource();
    expect(source).toMatch(/t\.comprasSialtiDesc/);
  });

  it("renders at least 4 bespoke benefit bullets via dedicated i18n keys", () => {
    const source = readSource();
    expect(source).toMatch(/t\.comprasSialtiBullet1Title/);
    expect(source).toMatch(/t\.comprasSialtiBullet2Title/);
    expect(source).toMatch(/t\.comprasSialtiBullet3Title/);
    expect(source).toMatch(/t\.comprasSialtiBullet4Title/);
  });

  it("renders a CTA (wa.me or #contacto), not hardcoded label text", () => {
    const source = readSource();
    expect(source).toMatch(/wa\.me|#contacto/);
    expect(source).toMatch(/t\.comprasSialtiCtaLabel/);
  });
  ...
  it("frames compliance generally, without over-specifying regulatory acronym details", () => {
    const source = readSource();
